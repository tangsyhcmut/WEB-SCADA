...
function ValveA3Pop() {
  ...
  const [stateVA3,setStateVA3] = useState([]);
...
useEffect(() => {
  ...
  const btnSetClick =async()=>{
    ...
    await socket.emit('VA3_Mode',mode)}

  
  ///Set Open
  const btnOpenClick =async()=>{
   
    await socket.emit('Button','VA3_OPEN')}
    ///Set Close
  const btnCloseClick =async()=>{
      
    await socket.emit('Button','VA3_CLOSE')}
    ///Set Reset
  const btnResetClick =async()=>{
      await socket.emit('Button','VA3_RESET')}
    ...
    return (
        ...
        <Row form>
          <Col>
            ...
            <img className="valvehighlight" src ={stateVA3 ?  redlighton : redlightoff } />
            <Label>Low limit </Label>
            <img className="valvelowlight" src ={stateVA3 ?  redlighton : redlightoff } />


            <Label>Opened</Label>
            <img className="valveopenlight" src ={stateVA3.OPENED? greenlighton : greenlightoff}/>
             <Label>Closed</Label>
            <img className="valvecloselight" src ={stateVA3.CLOSED ? greenlighton : greenlightoff}/>


             <Label> Fault </Label>
            <img className="valvefaultlight" src ={stateVA3.FAULT ?  redlighton : redlightoff } />
             </div>
            </FormGroup>
          </Col>
        </Row>
